...
CONFIG = {
    ...
    'staging': {
        'hosts': ['root@'],
        'dest': '/usr/local/lib/{{ project_name }}/',
        'env_name': 'staging',
        'requirements': 'deployment',
        'static': '/var/www/{{ project_name }}'
    },
    'live': {
        'hosts': ['root@'],
        'dest': '/usr/local/lib/{{ project_name }}/',
...
def build(patch=False):
    ...
    app_files = formic.FileSet(
        include=[
            '/requirements/**', '/{{ project_name }}/**', '/static/**', '/templates/**', '/manage',
        ], exclude=[
            '/{{ project_name }}/settings/environment.py', '*.pyc'
        ]
...
def deploy():
    ...
    with cd(env.dest):
        ...
        with cd(timestamp):
            remote_archive = '/tmp/{{ project_name }}-%s-%s.tar.gz' % (timestamp, version)
